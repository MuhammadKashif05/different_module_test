...
try:
    ...
    if not args.deleteuser:
        for user in users:
            ...
            print('              Group ID : {:<8} User ID : {:<8} Password  : {:<8} Card : {}'.format(user.group_id,
                                                                                                      user.user_id,
                                                                                                      user.password,
                                                                                                      user.card))
    ...
    if args.records:
        ...
        for att in attendance:
            i += 1
            print("ATT {:>6}: uid:{:>3}, user_id:{:>8} t: {}, s:{} p:{}".format(i, att.uid, att.user_id, att.timestamp,
                                                                                att.status, att.punch))
        print('    took {:.3f}[s]'.format(final - inicio))
    print('')
    print('--- sizes & capacity ---')
    conn.read_sizes()
    ...
    if args.live_capture:
        ...
        for att in conn.live_capture():  # using a generator!
            if att is None:
                ...
                print("ATT {:>6}: uid:{:>3}, user_id:{:>8} t: {}, s:{} p:{}".format(counter, att.uid, att.user_id,
